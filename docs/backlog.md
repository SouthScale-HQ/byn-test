...
- **Club partnership revenue share** — partner with lower-tier football clubs (e.g. League One, League Two, non-league) to drive user acquisition. Clubs promote BYN to their fanbase; users who sign up via a club's referral link or code join that club's "virtual BYN club". BYN shares a percentage of all revenue generated by those users (ad views, league slot purchases, future paid features) with the club on an ongoing basis. Gives clubs a new digital revenue stream with zero cost or risk; gives BYN a trusted acquisition channel with highly engaged sports fans.
  - **Revenue model**: BYN shares X% of all lifetime revenue from referred users — ad views, league slot purchases, future subscriptions. Club earns as long as their fans are active, creating incentive to keep promoting.
  - **Product**: each club gets a unique referral code, a pre-created branded private league, and a revenue dashboard (could be a tab in the southscale admin) showing referred users, activity and earnings.
  - **Scale path**: start with 2–3 clubs to prove the model → pitch to EFL clubs → eventually approach Premier League academy and community trust departments.
  - **Key questions to resolve**: revenue share % (suggest 20–30%), attribution window, minimum user threshold before payouts, handling users who support multiple clubs.
