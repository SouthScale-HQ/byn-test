...
1. **Auto-settlement job — ✅ DEPLOYED.** Single server-side result per round instead of per-browser `Math.random()`, scoped to EPL/La Liga/Championship/F1. Live via `settle-rounds` Edge Function on a `pg_cron` schedule (every 15 min). See Backend section for the one open follow-up (rotating a leaked `service_role` key). Confirmed decisions:
   ...
   **Two real gaps surfaced while writing this, need follow-up before deploy, not silently worked around:**
   - **F1 outcome matching — FIXED.** `market_outcomes.external_ref` now stores OpenF1's `driver_number` at market creation time (`persistenceManager.initRoundMarketsInDB`), flowing through from `f1-fixtures.js` → `oddsService.js` → `App.jsx`'s market seeding. Settlement matches on this ID, never on the label string. **`markets.external_session_key` — FIXED.** Confirmed missing via a direct schema query (`SELECT column_name FROM information_schema.columns WHERE table_name = 'markets'` — only had `id, event_id, market_type, liquidity_b, status, created_at, round_id`, no external reference at all). Added via `add-external-session-key-migration.sql`, populated in `persistenceManager.js` by parsing the session key out of `market.externalId`'s existing `f1-{sessionKey}` format. **Both migrations (`add-external-ref-migration.sql` and `add-external-session-key-migration.sql`) must be run before deploying the current `settle-rounds` Edge Function** — it assumes both columns exist.
...
2. **Wire leagues to existing Supabase tables** (Backend, rescoped 2026-07 after confirming live schema) — `groups`/`group_members` tables already exist in Supabase (confirmed via direct query), but `App.jsx` never uses them — leagues are still pure React state (`useState([])`), lost on refresh. Task is now "wire `createGroup`/`joinGroup`/league listing to the real tables," not "design and create a persistence layer."
3. ~~**Add `profiles.country`**~~ — **already exists.** Confirmed via direct schema query 2026-07. `profiles.date_of_birth` and `profiles.age_verified_at` also already exist (relevant to Phase 2 age gating below — schema is ready, only enforcement logic is missing).
...
- **Age gating** — schema already supports this (`profiles.date_of_birth`, `profiles.age_verified_at` confirmed live via direct query 2026-07) — only the enforcement logic is missing: capture DOB at signup, compute age server-side, reject signup if under 17. Not a schema task, just wiring.
...
- [ ] **`settle-rounds` Edge Function isn't in the repo** — deployed directly via the Supabase dashboard's browser editor (per how it was set up), so it has no git history and isn't backed up anywhere except Supabase's own storage. Worth adding `supabase/functions/settle-rounds/index.ts` to the repo now (matching what's actually deployed) purely for version control — doesn't need CLI deployment, just committing the file for safekeeping and future diffing.

- [ ] **`docs/database.sql` is stale — do not treat as current schema.** Confirmed 2026-07: it's missing `markets.round_id` (exists live, added after this file was written), and missing `round_fixtures`, `pending_deletions`, `reminders` tables entirely (all referenced directly in code). It also doesn't reflect `market_outcomes.external_ref` / `markets.external_session_key` (added this session). Either update this file to match live reality, or replace it with a note pointing at running `information_schema.columns` queries directly — a stale schema doc is worse than no doc, since it actively misleads. Also worth checking `docs/schema.md` for the same staleness, since it also predates several implementation pivots (e.g. describes seeding odds from The Odds API + de-vigging, per `decisions.md` — superseded by the later move to in-house probability models).

- [ ] **URGENT — rotate/replace leaked `service_role` key** — the legacy `service_role` key was pasted in plaintext during the cron setup session (2026-07). Recommended fix: don't attempt to rotate the legacy key directly (its relationship to the newer JWT Signing Keys rotation UI is ambiguous — rotating the signing key affects newly-issued Auth tokens, not necessarily the static legacy `anon`/`service_role` strings). Instead, **migrate off it entirely**: generate a new `sb_secret_...` key via Supabase's "Publishable and secret API keys" tab, then update everywhere `SUPABASE_SERVICE_ROLE_KEY` is used — Vercel env vars for both `byn-test` and `southscale-web`, and the `settle-rounds-every-15-min` cron job's Authorization header (`cron.alter_job()` or drop/recreate). Once nothing references the old key, the leak becomes moot regardless of whether it's technically still valid. **First task to pick up next session.**
...
- [x] **Scheduled jobs — auto-settlement** — deployed. `settle-rounds` Edge Function live, scheduled via `pg_cron` every 15 min (job `settle-rounds-every-15-min`). Covers EPL/La Liga/Championship/F1 (see Path to launch Phase 1 for full design history). Lockout enforcement and weekly topup at round start are still not built — only auto-settlement is done.
- [ ] **Wire leagues to existing Supabase tables** — `groups`/`group_members` tables already exist in Supabase (confirmed via `information_schema.columns` query, 2026-07: `groups` has `id, name, creator_id, invite_code, status, max_members, sponsor_org_id, is_sponsored, created_at`; `group_members` has `group_id, user_id, joined_at`). But `App.jsx`'s `createGroup`/`joinGroup` and league listing still only touch local React state (`useState([])`), never the DB — leagues don't survive a refresh. Rescoped from "build a persistence layer" to "wire existing tables into the app."
- [x] **`profiles.country`** — confirmed already exists live (2026-07 schema query). No migration needed.
